...
class TestSummaryService:
    ...
    def test_get_portfolio_summary_empty(self, db_session: Session) -> None:
        ...
        service = SummaryService(db_session, TEST_USER_ID)
    ...
    def test_get_portfolio_summary_with_holdings(self, db_session: Session) -> None:
        ...
        repo.bulk_create([_make_transaction()], user_id=TEST_USER_ID)
        service = SummaryService(db_session, TEST_USER_ID)
        result = service.get_portfolio_summary()
...
def _make_cash_account(
    session: Session, balance: float = 500.0, user_id: int = TEST_USER_ID
) -> CashAccount:
    acct = CashAccount(
        name="Test Account",
...
class TestWealthSummaryService:
    ...
    def test_empty_portfolio_has_no_data(self, db_session: Session) -> None:
        ...
        result = SummaryService(db_session, TEST_USER_ID).get_wealth_summary()
...
class TestRiskAssessmentService:
    ...
    def test_empty_portfolio_returns_moderate_no_data(
        ...
        service = SummaryService(db_session, TEST_USER_ID)
        result = service.get_risk_assessment("moderate")
        assert result.has_data is False
    ...
    def test_user_preference_is_preserved(self, db_session: Session) -> None:
        ...
        service = SummaryService(db_session, TEST_USER_ID)
        result = service.get_risk_assessment("conservative")
    ...
    def test_aligned_false_when_mismatch(self, db_session: Session) -> None:
        ...
        _make_cash_account(db_session, balance=1000.0)  # conservative portfolio
        service = SummaryService(db_session, TEST_USER_ID)
        result = service.get_risk_assessment("aggressive")
        assert result.is_aligned is False
...
class TestRiskEndpoint:
    ...
    def test_aggressive_portfolio_detected(self, client, db_session: Session) -> None:
        ...
        uid = _user_id_by_email(db_session, "risk_agg2@test.com")
        _make_crypto(db_session, price=500.0, qty=3.0, user_id=uid)  # 1500 high
        _make_cash_account(db_session, balance=100.0, user_id=uid)  # 100 low
        response = client.get(
            "/summary/risk",
            headers={"Authorization": f"Bearer {token}"},
        )
...
class TestDiversificationService:
    ...
    def test_empty_portfolio_is_diversified_no_data(self, db_session: Session) -> None:
        ...
        service = SummaryService(db_session, TEST_USER_ID)
        result = service.get_diversification_recommendations()
        assert result.has_data is False
...
class TestDiversificationEndpoint:
    ...
    def test_concentrated_portfolio_has_recommendation(
        ...
        uid = _user_id_by_email(db_session, "div_conc2@test.com")
        _make_cash_account(db_session, balance=1000.0, user_id=uid)
        response = client.get(
            "/summary/diversification",
            headers={"Authorization": f"Bearer {token}"},
        )
    ...
    def test_diversified_portfolio_no_recommendations(
        ...
        uid = _user_id_by_email(db_session, "div_ok2@test.com")
        _make_cash_account(db_session, balance=400.0, user_id=uid)
        _make_crypto(db_session, price=200.0, qty=2.0, user_id=uid)  # 400 PLN
        response = client.get(
            "/summary/diversification",
            headers={"Authorization": f"Bearer {token}"},
        )
...
class TestEmergencyFundService:
    ...
    def test_empty_portfolio_returns_no_data(self, db_session: Session) -> None:
        ...
        service = SummaryService(db_session, TEST_USER_ID)
        result = service.get_emergency_fund(monthly_expenses=1000.0)
        assert result.has_data is False
    ...
    def test_monthly_expenses_zero_returns_good_zero_months(
        ...
        service = SummaryService(db_session, TEST_USER_ID)
        result = service.get_emergency_fund(monthly_expenses=0.0)
        assert result.months_covered == 0.0
        assert result.status == "good"
    ...
    def test_cash_and_bonds_values_correct(self, db_session: Session) -> None:
        ...
        service = SummaryService(db_session, TEST_USER_ID)
        result = service.get_emergency_fund(monthly_expenses=500.0)
    ...
    def test_emergency_fund_equals_cash_plus_bonds(self, db_session: Session) -> None:
        ...
        service = SummaryService(db_session, TEST_USER_ID)
        result = service.get_emergency_fund(monthly_expenses=800.0)
        assert result.emergency_fund == round(result.cash_value + result.bonds_value, 2)
...
class TestEmergencyFundEndpoint:
    ...
    def test_cash_present_returns_data(self, client, db_session: Session) -> None:
        ...
        uid = _user_id_by_email(db_session, "ef_cash@test.com")
        _make_cash_account(db_session, balance=3000.0, user_id=uid)
        response = client.get(
            "/summary/emergency-fund",
            headers={"Authorization": f"Bearer {token}"},
        )
